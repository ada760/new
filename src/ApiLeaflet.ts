...
let selectionMode: "departure" | "arrival" | null = null
let departureCountryData: CountryData | null = null
let arrivalCountryData: CountryData | null = null
...
function calculateDistance(): void {
  ...
  if (distanceKmInput) distanceKmInput.value = `${Math.round(realDistance)}`
...
document.addEventListener("DOMContentLoaded", (): void => {
  ...
  typeTransportSelect?.addEventListener("change", function (): void {
    const selectedType = this.value as TypeCargaison // Caster en TypeCargaison

    // Définir poidsMax automatiquement en fonction du type de transport
    let newPoidsMax: number
    switch (selectedType) {
  ...
  formCargaison?.addEventListener("submit", async (e: Event): Promise<void> => {
    ...
    const transportType = (document.getElementById("typeTransport") as HTMLSelectElement)?.value as TypeCargaison
    if (!transportType) {
